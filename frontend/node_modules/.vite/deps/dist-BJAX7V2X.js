...
import {
  ...
  toHex2,
  waitForCallsStatus
} from "./chunk-HRFSUN5U.js";
import {
  AccountNotFoundError,
  defineChain,
  formatLog,
  formatTransactionReceipt
} from "./chunk-2U3QCKUE.js";
import {
  concat as concat2,
  decodeFunctionData,
  encodeFunctionData,
...
var require_abi = __commonJS({
  "node_modules/@coinbase/wallet-sdk/dist/vendor-js/eth-eip712-util/abi.cjs"(exports, module) {
    ...
    function rawEncode(types2, values) {
      ...
      for (var i in types2) {
        var type6 = elementaryName(types2[i]);
        var value = values[i];
...
function pad2(bytes2, options = {}) {
  ...
  for (let i = 0; i < size6; i++) {
    ...
    paddedBytes[padEnd ? i : size6 - i - 1] = bytes2[padEnd ? i : bytes2.length - i - 1];
  }
  return paddedBytes;
}
...
function ripemd1603(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes2 = ripemd1602(from4(value));
  if (as === "Bytes")
    return bytes2;
  return fromBytes(bytes2);
}
function sha2563(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes2 = sha2562(from4(value));
  if (as === "Bytes")
    return bytes2;
  return fromBytes(bytes2);
}
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i) => {
        if (Array.isArray(args_[i]))
          return args_[i].map((_2, j) => encode10(param, args_[i][j]));
...
function ecdsa(Point, hash9, ecdsaOpts = {}) {
  ...
  class Signature {
    ...
    toHex(format9) {
      return bytesToHex2(this.toBytes(format9));
    }
    // TODO: remove
    assertValidity() {
    }
  ...
  function inner(node) {
    ...
    while (level.length > 1) {
      ...
      for (let i = 0; i < level.length; i += 2)
        level_.push(hash3(concat4(level[i], level[i + 1])));
      level = level_;
    }
...
function to2(blobs, to3) {
  ...
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x) => fromHex2(x)) : blobs;
...
function toProofs(blobs, options) {
  ...
  const as = options.as ?? (typeof blobs[0] === "string" ? "Hex" : "Bytes");
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x) => fromHex2(x)) : blobs;
  const commitments = typeof options.commitments[0] === "string" ? options.commitments.map((x) => fromHex2(x)) : options.commitments;
  const proofs = [];
...
function fromCreate(options) {
  const from30 = fromHex2(from6(options.from));
  let nonce = fromNumber3(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from6(`0x${keccak2563(fromBytes3([from30, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from6(slice3(keccak2563(concat4(fromHex2("0xff"), from30, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function namehash(name) {
  ...
  for (let i = labels.length - 1; i >= 0; i -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex2(hashFromEncodedLabel) : keccak2563(fromString2(labels[i]), { as: "Bytes" });
...
function verify2(options) {
  ...
  return secp256k1.verify(signature, from4(payload), toBytes4(publicKey), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
function sign4(options) {
  const { extraEntropy: extraEntropy2 = extraEntropy, hash: hash9, payload, privateKey } = options;
  const { r, s, recovery } = secp256r1.sign(payload instanceof Uint8Array ? payload : fromHex2(payload), privateKey instanceof Uint8Array ? privateKey : fromHex2(privateKey), {
    extraEntropy: typeof extraEntropy2 === "boolean" ? extraEntropy2 : from3(extraEntropy2).slice(2),
    lowS: true,
    ...hash9 ? { prehash: true } : {}
  });
...
function createWalletSendCallsRequest({ calls, from: from30, chainId, capabilities }) {
...
@noble/hashes/esm/utils.js:
  (*! noble-hashes - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=dist-BJAX7V2X.js.map
